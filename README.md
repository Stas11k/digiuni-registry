...
### Робота з факультетами та кафедрами:
Можна:
- створювати
- видаляти
- редагувати
...
### Звіти:
У меню звітів можна:
- побачити студентів відсортованих за курсами(у меню студентів)
- відсортувати студентів/викладачів
- переглянути дані по кафедрі або факультету

### Робота з файлами
Програма автоматично зберігає дані у файл, як тільки ви зробили якісь зміни, відразу все зберігається у файл.

### Мережевий режим
Програма підтримує роботу через сервер:
- сервер запускається окремо
- дані передаються у форматі JSON

### Важливо
- якщо ввести неправильне значення - програма попросить повторити
- деякі функції недоступні без прав (залежить від ролі)
